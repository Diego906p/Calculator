...
function buildBoletaPDF(worker, result, anio, mes) {
  ...
  doc.text(
    'Generado: ' + new Date().toLocaleDateString('es-PE') + ' · CALCULATOR v15',
...
function exportarPDFSingle(idx) {
  ...
  const doc   = buildBoletaPDF(item.worker, item.result, anio, mes);
  const fname = anio + '_' + String(mes + 1).padStart(2, '0') + '_BP_' +
                _normNombre(item.worker.nombre) + '.pdf';
  doc.save(fname);
}
...
async function exportarZIP() {
  if (!window._masResults || _masResults.length === 0) {
  ...
  showToast('Generando PDFs...');

  const JSZip = window.JSZip;
  if (!JSZip) { showToast('Error: JSZip no disponible'); return; }

  const zip    = new JSZip();
  const folder = zip.folder('Boletas_' + anio + '_' + mesStr);

  for (const item of _masResults) {
    if (item.estado !== 'ok') continue;
    const doc   = buildBoletaPDF(item.worker, item.result, anio, mes);
    const bytes = doc.output('arraybuffer');
    const fname = anio + '_' + mesStr + '_BP_' + _normNombre(item.worker.nombre) + '.pdf';
  ...
  a.href     = url;
  a.download = 'Boletas_' + anio + '_' + mesStr + '.zip';
  a.click();
  URL.revokeObjectURL(url);
  showToast('ZIP descargado ✓');
}

// ── verBoletaIndividual (preview modal para modo individual) ─────────────────
