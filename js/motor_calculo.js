...
function calcularForaneo(params) {
  ...
  const alimentacion = ALIM_DIA   * diasViat;
  const alojamiento  = ALOJ_DIA   * diasViat;
  // movilidadMes es el monto mensual fijo; se paga si trabajó campo al menos un día
  const movilidad    = diasViat > 0 ? (movilidadMes || 0) : 0;
